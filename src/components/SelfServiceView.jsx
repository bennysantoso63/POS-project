...
export default function SelfServiceView({ 
  ...
  return (
    <div className="flex h-screen bg-brand-bg text-brand-text font-sans selection:bg-blue-500/30 transition-colors">
      
      {/* Kolom Kiri: Scanner Hub (60%) */}
      <div className="w-[60%] border-r border-brand-border p-8 flex flex-col relative">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-light text-brand-text tracking-wide">POS <span className="font-semibold text-blue-500">Mandiri</span></h1>
        ...
        <div className="flex-1 bg-brand-card/30 rounded-3xl border border-brand-border flex items-center justify-center relative overflow-hidden group shadow-2xl">
          <div className="absolute inset-0 bg-gradient-to-b from-brand-bg/50 to-transparent z-10" />
        ...
        <div className="mt-8">
          ...
          <div className="flex gap-4">
            <input 
              ...
              className="flex-1 bg-brand-card border border-brand-border rounded-2xl px-6 py-5 text-brand-text font-mono text-xl shadow-inner focus:outline-none focus:border-blue-500 transition-all"
            />
            <button 
      ...
      <div className="w-[40%] bg-brand-bg flex flex-col relative">
        
        {/* Ling-Ling AI Dynamic Nudge */}
        <div className="h-48 border-b border-brand-border p-8 flex items-center justify-center">
          {lingLingMessage ? (
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-4 custom-scrollbar">
          {cart.length === 0 ? (
            ...
            cart.map((item, index) => (
              <div key={index} className="bg-brand-card/50 p-5 rounded-[1.5rem] flex justify-between items-center border border-brand-border/50 hover:border-brand-border transition-all animate-in fade-in slide-in-from-right-4">
                ...
                <p className="font-black text-brand-text text-lg tracking-tighter">{formatRupiah(item.price * item.qty)}</p>
              </div>
            ))
          )}
        </div>
